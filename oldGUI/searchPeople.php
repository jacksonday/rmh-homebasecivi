				...
				<?PHP
					if($_POST['s_submitted']){
						...
						$query = "SELECT * FROM dbPersons WHERE first_name LIKE '%".$fns."%' " .
								...
								"AND email LIKE '%".$ems."%' ";			
						if ($_POST['s_status']!=="") $query .= "AND status LIKE '%".$_POST['s_status']."%' ";
		...
		//				echo resetall_passwords();
						...
						if($_SESSION['access_level']>=2){
							...
							if(mysql_num_rows($result))
								echo('<tr><td class=searchResults><strong>Name</strong></td><td class=searchResults><strong>View</strong></td><td class=searchResults><strong>Edit</strong></td>');
							while($thisRow = mysql_fetch_array($result, MYSQL_ASSOC)){
								//if($thisRow['id']=='admin') continue;
								echo("<tr><td class=searchResults>".$thisRow['last_name'].", ".$thisRow['first_name']."</td><td class=searchResults><a href=\"viewPerson.php?id=".$thisRow['id']."\">view</td><td class=searchResults><a href=personEdit.php?id=".$thisRow['id'].">edit</a></td></tr>");
							}
							echo("</table></p>");
						}
						else{
