...
function generate_scl($id) {
    ...
    connect();
    $query = "SELECT * FROM dbPersons WHERE (status = 'active' AND type LIKE '%" . $venue .
            "%' AND availability LIKE '%" . $day . $time . "%') ORDER BY last_name,first_name";
    ...
    for ($i = 0; $i < mysql_num_rows($result); ++$i) {
        ...
        if (!$match) {
            $persons[] = array($row[0], $row[1], $row[2], $row[7], $row[8], "", "", "?");
        }
    }
...
function do_scl_index($id) {
    ...
    for ($i = 0; $i < mysql_num_rows($result); ++$i) {
        ...
        if (($row[2] == "closed" && $_GET['archive'] == "true") || $row[2] == "open") {
            ...
            . date("l, F jS, Y", mktime(0, 0, 0, substr($row[0], 0, 2), substr($row[0], 3, 2), substr($row[0], 6, 2)))
            . " from " . do_name($row[0]) .
            "</a></td>
...
function view_scl($id) {
    ...
    echo "<table width=\"700\" align=\"center\" border=\"1px\">
				<tr><td colspan=\"5\" align=\"center\"><b>Sub Call List for " . date("l, F jS, Y", mktime(0, 0, 0, substr($id, 0, 2), substr($id, 3, 2), substr($id, 6, 2))) .
    " from " . do_name($id) . "</b></td></tr>";
    ...
    if ($status == "closed") {
        for ($i = 0; $i < count($persons); ++$i) {
            echo "<tr><td>&nbsp;<a href=\"viewPerson.php?id=" . $persons[$i][0] . "\">" . $persons[$i][1] . " " . $persons[$i][2] . "</a></td>
...
function process_edit_scl($post) {
    ...
    if ($new_acceptances > $vacancies) {
        ...
        update_dbShifts($shift);
        for ($j = 0; $j < count($accepted_people); ++$j) {
            add_log_entry('<a href=\"viewPerson.php?id=' . $_SESSION['_id'] . '\">' .
                    $_SESSION['f_name'] . ' ' . $_SESSION['l_name'] . '</a> assigned <a href=\"viewPerson.php?id=' . $persons_new[$accepted_people[$j]][0] . '\">' .
...
function do_name($id) {
    $venue = substr($id, strlen($id) - 3);
    $id = substr($id, 9);
    $id = substr($id, 0, strlen($id) - 3);
    $venue_names = array("Rec" => "Reception", "Fam" => "House Family Room", "Van" => "Van", "Kit" => "Kitchen",
        "Off" => "Office", "Lin" => "Linens", "Chr" => "Christiana Family Room", "Dup" => "Dupont Family Room");
    $shift_names = array("9-12" => "9am to 12pm",
        "12-15" => "12pm to 3pm",
        "15-18" => "3pm to 6pm",
        "18-21" => "6pm to 9pm",
        "12-14" => "12pm to 2pm", "14-16" => "2pm to 4pm",
        "16-18" => "4pm to 6pm", "18-20" => "6pm to 8pm"
    );
    return $shift_names[$id] . " " . $venue_names[$venue];
}
